...
if not api_key:
    print(
        "⚠️  Warning: OPENROUTER_API_KEY not found in environment. LLM queries will fail."
    )
else:
...
def parse_syllabus_to_chunks(filepath: Path) -> list[dict]:
    ...
    for line in lines:
        ...
        if (
            "course id" in line_clean.lower()
            or "course_id" in line_clean.lower()
        ):
            parts = line_clean.split(":")
            if len(parts) > 1:
                course_id = parts[1].strip()
        if (
            "instructor" in line_clean.lower()
            or "professor" in line_clean.lower()
        ):
            parts = line_clean.split(":")
            if len(parts) > 1:
    ...
    print(
        f"  └─ Extracted {len(final_chunks)} semantic chunks (Course: {course_id}, Prof: {professor})"
    )
    return final_chunks
...
def main():
    ...
    syllabi_dir = DATA_DIR / "sample_data" / "syllabi"
    txt_files = glob.glob(str(syllabi_dir / "*.txt"))

    if not txt_files:
        print(
            f"❌ Error: No text syllabi found in {syllabi_dir}. Please create them first."
        )
        sys.exit(1)
    ...
    collection = db_client.create_collection(
        name="syllabi_collection", embedding_function=embedding_fn
    )
    ...
    results_md.append("> **Date**: June 10, 2026")
    results_md.append("> **Status**: RUN COMPLETED SUCCESSFULLY")
    ...
    for idx, q in enumerate(queries, 1):
        ...
        for i in range(len(retrieved_docs)):
            ...
            print(
                f"  [{i + 1}] Course: {meta['course_id']} | Section: {meta['section_name']} (Similarity: {score:.4f})"
            )
            context_parts.append(
                f"Source: {meta['course_id']} ({meta['section_name']})\nContent:\n{doc}"
            )
