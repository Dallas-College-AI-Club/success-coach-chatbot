"""Compose catalog, CV and section facts into shared knowledge rows.

The CLI loads an offline SQLite reference index; production deliveries use
load_catalog_to_neon after embedding and validation. SQLite filter columns
mirror the PostgreSQL generated columns and its FTS index supports local checks.

    python -m dallasai.pipeline.build_knowledge --rows rows.json --sqlite knowledge.db
"""
...
def _canonical(chunk_text: str, facts: Any, metadata: dict) -> str:
    return json.dumps(
        {"t": chunk_text, "f": facts, "m": metadata},
        sort_keys=True,
        separators=(",", ":"),
        ensure_ascii=False,
    )
...
def _section_metadata(sec: dict) -> dict:
    ...
    return {
        ...
        "schema_version": "facts-section-v2",
    }


def compose_sections(
    sections: list[dict], syllabus_by_key: dict[tuple, dict]
) -> list[dict]:
    ...
    `syllabus_by_key`: (professor_slug, course_code, modality) -> {facts, source_url}.
    """
    rows: list[dict] = []
    ...
    for s in sections:
        key = (
            slug(s.get("professor", "")),
            norm_course_code(s.get("course_code", "")),
            s.get("modality"),
        )
    ...
    for s in sections:
        ...
        rows.append(
            _row(s["source_url"], 0, s["chunk_text"], facts, _section_metadata(s))
        )
    return rows
...
def compose_course(cf: dict, source_url: str, catalog_year: str) -> dict:
    ...
    meta = {
        ...
        "module": "degree_planning",
        "course_code": cc,
        "subject": cc.split(" ")[0] if cc else None,
        "catalog_year": catalog_year,
    }
    credits = cf.get("credit_hours")
    requisites = cf.get("requisites_raw") or (
        "details unavailable; absence of requirements is not established"
    )
    text = (
        f"{cc} — {cf.get('title') or ''} "
        f"({credits if credits is not None else '?'} cr). "
        f"{cf.get('description', '') or ''} "
        f"Prerequisite record: {requisites}. "
        + (
            "Texas Common Course Number (generally transfers to Texas public "
            "universities - verify with the receiving school). "
            if cf.get("tccn")
            else ""
        )
    )
...
def compose_program(pf: dict, source_url: str, catalog_year: str) -> dict:
    ...
    text = (
        f"{pf.get('name') or ''} ({pf.get('award_type') or 'award type not stated'}), "
...
def compose_cv(cf: dict, source_url: str) -> dict:
    meta = {
        "doc_type": "cv",
        "module": "degree_planning",
        "instructor_slug": slug(cf.get("name", "")),
        "professor": cf.get("name"),
    }
    text = (
        f"{cf.get('name', '')}"
...
def load_sqlite(rows: list[dict], db_path: Path) -> None:
    ...
    with closing(sqlite3.connect(db_path)) as con, con:
            ...
            con.execute(
                f"""INSERT INTO knowledge_entry
                    ...
                    ON CONFLICT(source_url, chunk_index) DO UPDATE SET
                      ...
                      {", ".join(f"{c}=excluded.{c}" for c in FILTER_COLS)}""",
                [
                    r["source_url"],
                    r["chunk_index"],
                    r["content_hash"],
        ...
        # Rebuild the small offline search index from authoritative row IDs. SQLite's
        # lastrowid is not the updated row ID after ON CONFLICT, so incremental inserts
        # here previously duplicated or attached text to the wrong record on reruns.
        con.execute("INSERT INTO ke_fts(ke_fts) VALUES ('rebuild')")
        con.commit()
...
if __name__ == "__main__":
    ...
    syl_by_key = {
        tuple(k.split("|")): v for k, v in data.get("syllabus_by_key", {}).items()
    }
    rows: list[dict] = []
