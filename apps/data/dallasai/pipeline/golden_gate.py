...
EVERY prompt, model, or schema change.
    ...
    python -m pipeline.golden_gate --raw-root <raw> [--gate-dir gate]
...
def main(argv=None) -> None:
    ...
    ap.add_argument("--gate-dir", type=Path,
                    default=Path(__file__).resolve().parent.parent / "gate")
    ...
    for case_dir in sorted(p for p in args.gate_dir.iterdir() if p.is_dir()):
        ...
        rel = rel_file.read_text(encoding="utf-8").strip() if rel_file.exists() else REL[case]
        context = json.loads((case_dir / "context.json").read_text(encoding="utf-8"))
        expected = json.loads((case_dir / "expected.json").read_text(encoding="utf-8"))
        document = html_to_text((args.raw_root / rel).read_text(encoding="utf-8", errors="replace"))
        ...
        if res.status != "ok":
            print(f"[GATE FAIL] {case}: extraction status {res.status} "
                  f"(errors: {res.validation_errors})")
            n_fail += 1
        elif res.data != expected:
            paths = diff_paths(res.data, expected)
