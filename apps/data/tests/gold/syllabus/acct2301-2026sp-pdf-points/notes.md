...
- [ ] **`policies.extra_credit` punctuation**: the value joins a multi-line table cell; the `;` separators and the mid-value `.` were inserted at the line-join seams and are NOT in the PDF text. Per the README's VERBATIM fold these separators fold away — verify the *content tokens* against `input.txt`, not the punctuation.
...
`input.txt` IS the pipeline converter's output: regenerated 2026-07-18 via
`pipeline.extract.pdf_to_text()` (pypdf 6.14.2) from the source PDF — reproduce with
`python -c "from pipeline.extract import pdf_to_text; from pathlib import Path; print(pdf_to_text(Path('sample_syllabi/Prin of Financial Accounting _ Syllabus _ Concourse.pdf')))"`.
It includes the repeated Concourse page-header/URL lines a real batch run sees; the
human verifier checks the fields against THIS rendering (the traps survive it: the
empty "Late Work" heading, the checksum row, the makeup policy inside the Final Exams
notes). A converter or pypdf-version change that alters this text marks the case
`stale` (excluded from gating) until regenerated and re-verified.
