...
| VERBATIM | `raw_label`, `policies.*`, `office_hours`, `instructor_contact`, `required_materials[].raw_text` | normalized equality **and** grounding in `input.txt` (no fuzzy — token overlap rates a policy ≈ its negation). The normalization fold, for both sides: NFKC (folds PDF ligatures like ﬁ→fi), casefold, newlines→spaces, whitespace collapse, and punctuation inserted at line-join seams (a `;` or `.` where the source broke a list across lines) folds away. Grounding = the folded gold value appears as a token sequence in the folded `input.txt` |
