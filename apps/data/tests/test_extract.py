...
def test_registry_stamp_accepts_wired_doc_types_and_methods():
    for doc_type in ex.DOC_SCHEMAS:
        for method in ("api", "local_ollama", "local_lmstudio", "claude_code_session", "seed"):
...
def test_parse_json_response_tolerates_trailing_prose():
    ...
    assert ex.parse_json_response(
        'Sure! Here it is:\n```json\n{"confidence": "high"}\n```\n'
        'Let me know if you need anything else.') == {"confidence": "high"}
    assert ex.parse_json_response(
        'Here you go: {"confidence": "high"} Hope that helps!') == {"confidence": "high"}
...
def test_build_prompt_guards_unimplemented_examples_placeholder(monkeypatch, tmp_path):
...
def test_extract_retries_with_errors_then_succeeds(monkeypatch):
    ...
    res = ex.extract("syllabus", "doc text", {}, extractor_setting="lmstudio:test-model")
...
def test_extract_exhausted_retries_quarantines(monkeypatch):
    monkeypatch.setattr(ex, "call_model",
                        lambda *a, **k: '{"confidence": "very sure"}')
    res = ex.extract("syllabus", "doc text", {}, extractor_setting="lmstudio:test-model")
...
def test_extract_uses_doc_type_token_budget(monkeypatch):
    ...
    def fake_model(provider, model, prompt, max_tokens=ex.DEFAULT_MAX_TOKENS):
        ...
        return ('{"program_code": "CORE-42", "name": "Core Curriculum", '
                '"confidence": "high"}')                  # fully schema-valid

    monkeypatch.setattr(ex, "call_model", fake_model)
    res = ex.extract("program_map", "doc text", {}, extractor_setting="lmstudio:test-model")
...
def test_persist_quarantine_writes_full_provenance(tmp_path):
    res = ex.ExtractionResult(
        ...
        validation_errors=None, extractor="test-model",
        extraction_method="local_lmstudio", schema_version="1", attempts=3,
        raw_responses=['{"confidence": "low"}'])
    src = Path("raw/syllabi/2026SP/ACCT-2301-24.html")
    ctx = {"course_code": "ACCT 2301", "term_code": "2026SP"}
    path = ex.persist_quarantine(res, tmp_path, "2026SP-ACCT-2301-24",
...
def test_persist_quarantine_never_overwrites_on_collision(tmp_path):
    res = ex.ExtractionResult(status="needs_review", doc_type="syllabus",
                              data={"confidence": "low"})
