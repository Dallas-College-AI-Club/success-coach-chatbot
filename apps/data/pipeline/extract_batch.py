...
never from the document body.
    ...
    python -m pipeline.extract_batch --raw-root <path-to-raw> \
...
from .extract import (extract, extract_manual, html_to_text, pdf_to_text,
                      persist_quarantine)

KIND_TO_DOC_TYPE = {"syllabus": "syllabus", "cv": "cv",
                    "course": "course", "program": "program_map"}
...
def _context(entry: dict) -> dict:
    ...
    if kind == "cv":
        return {"professor": entry.get("professor")}
...
def main(argv: Optional[list[str]] = None) -> None:
    ...
    ap.add_argument("--doc-type", choices=sorted(set(KIND_TO_DOC_TYPE.values())),
    ...
    ap.add_argument("--quarantine-dir", type=Path, default=None,
                    help="default <out>/../quarantine")
    ap.add_argument("--refresh", action="store_true",
                    help="re-extract even when the output file exists")
    ap.add_argument("--replay-map", type=Path,
    ...
    for entry in iter_manifest(args.raw_root, args.manifest_glob):
        ...
        if rel in replay:
            ...
            if src.suffix.lower() == ".pdf":
                ...
                document = html_to_text(src.read_text(encoding="utf-8", errors="replace"))
            res = extract(doc_type, document, context)

        if res.status == "ok" and res.data is not None:
            dest.parent.mkdir(parents=True, exist_ok=True)
            dest.write_text(json.dumps({
                "facts": res.data, "context": context,
                "source_url": entry.get("source_url"), "raw_path": entry["raw_path"],
                "sha256": entry.get("sha256"), "fetched_at": entry.get("fetched_at"),
                "doc_type": doc_type, "extractor": res.extractor,
                "extraction_method": res.extraction_method,
                "prompt_version": res.prompt_version, "attempts": res.attempts,
    ...
    print("\n" + "  ".join(f"{k}={v}" for k, v in sorted(stats.items())) or "nothing matched")
    if stats["quarantined"]:
        print(f"quarantine -> {qdir} (review 100% of these before any load; see issue #72)")
