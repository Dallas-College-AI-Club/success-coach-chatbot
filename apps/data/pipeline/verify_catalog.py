...
moved to quarantine semantics (reported, excluded) — never silently loaded.
    ...
    python -m pipeline.verify_catalog --facts out/facts --raw-root <raw>
...
NOISE = ["Add to Portfolio", "opens a new window", "Catalog Navigation",
         "My Portfolio", "Print (", "Whole Word/Phrase"]
...
def codes_in(folded: str) -> set[str]:
    ...
    found = set()
    for m in re.finditer(r"\(?([A-Z]{2,4})\)?\s?(\d{4})((?:\s*(?:,|and|or)\s*\d{4})*)", folded):
        subj = m.group(1)
...
def check_course(f: dict, src: str, folded: str) -> list[str]:
    errs = []
    m = re.search(re.escape(f["course_code"])
                  + r" - .{0,200}?\((\d+(?:-\d+)?) Credit Hours?\)", folded)
    if not m and "Non-Credit Course" in src and re.search(
            re.escape(f["course_code"]) + r" - ", folded):
        m = None            # non-credit pages print no "(N Credit Hours)" suffix
    elif not m:
        errs.append(f"course_code/title line not found for {f['course_code']}")
    elif ("-" not in m.group(1) and f.get("credit_hours") is not None
    ...
    if f.get("description") and fold(f["description"]) not in folded:
        ...
        def tokstream(s):
            ...
            return re.sub(r"\b(\d+) (st|nd|rd|th)\b", lambda g: g.group(1) + g.group(2), j)
        if tokstream(f["description"]) not in tokstream(folded):
            errs.append("description content tokens differ from source (not seam punctuation)")
    return errs
...
def check_program(f: dict, src: str, folded: str) -> list[str]:
    ...
    for g in f.get("groups") or []:
        if not g.get("courses") and g.get("slot_kind") in ("fixed", "choose"):
            errs.append(f"group {g.get('name')!r} is {g.get('slot_kind')} but lists no courses")
        for c in g.get("courses") or []:
            if CODE.fullmatch(c) and c not in folded:
                errs.append(f"group {g.get('name')!r}: course {c!r} not printed in source")
        if g.get("options_exhaustive") is False and not re.search(
                r"[Oo]ther options (exist|may exist)", src):
            errs.append(f"group {g.get('name')!r}: options_exhaustive=false without the printed wording")
    ...
    if f.get("total_credits") and all(isinstance(x, int) for x in stated) and stated:
        if sum(stated) not in (f["total_credits"],):
            ...
            errs.append(f"WARN group credits sum {sum(stated)} != total_credits {f['total_credits']}"
...
def main(argv=None) -> None:
    ...
    ap.add_argument("--facts", type=Path, required=True, help="facts output dir (from extract_batch)")
    ...
    for sub, checker in (("course", check_course), ("program_map", check_program)):
        for p in sorted((args.facts / sub).glob("*.json")):
            ...
            errs += [f"nav/UI noise leaked into facts: {t!r}"
                     for t in NOISE for v in _values(env["facts"]) if t in v]
            accepted = json.loads((Path(__file__).resolve().parent.parent / "gate" /
                "adjudicated_source_anomalies.json").read_text(encoding="utf-8"))                 if (Path(__file__).resolve().parent.parent / "gate" /
                    "adjudicated_source_anomalies.json").exists() else {}
