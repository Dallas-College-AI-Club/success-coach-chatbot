...
def check_course(f: dict, src: str, folded: str) -> list[str]:
    errs = []
    m = re.search(re.escape(f["course_code"]) + r" - .{0,90}?\((\d+) Credit Hours?\)", folded)
    if not m:
        errs.append(f"course_code/title line not found for {f['course_code']}")
    elif f.get("credit_hours") is not None and int(m.group(1)) != f["credit_hours"]:
    ...
    if f.get("description") and fold(f["description"]) not in folded:
        errs.append("description is not a verbatim substring of the source")
    return errs
