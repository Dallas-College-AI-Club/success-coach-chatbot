...
export const focus: Mode = {
  ...
  Wizard: FocusShell,
  skin: {
    page: "coach-page coach-focus flex min-h-dvh w-full flex-col items-center justify-center gap-5 bg-[#EDF2F7] p-4 text-[#2B3A4A] md:p-8 [--ring:#ec5f56]",
    shell: "mx-auto flex w-full max-w-3xl flex-col items-stretch gap-4",
    sectionLabel:
      "coach-accent font-mono text-sm tabular-nums tracking-wide text-[#4A6076]",
    heading:
      "coach-heading text-3xl font-semibold text-[#2B3A4A] outline-none md:text-4xl",
    helper: "coach-helper text-sm text-[#2B3A4A]/75",
    option:
    ...
    optionCheck:
      "coach-accent shrink-0 text-lg leading-none text-[#ec5f56] opacity-0 transition-opacity group-data-[state=on]/opt:opacity-100",
    chip: "coach-chip inline-flex items-center gap-1.5 rounded-full bg-[#7C93AC]/15 px-3 py-1 text-sm text-[#2B3A4A] transition-colors hover:bg-[#7C93AC]/25 motion-safe:animate-in motion-safe:fade-in motion-safe:slide-in-from-left-2",
    chipCheck: "coach-accent text-[#5E7690]",
    primaryBtn:
      "coach-primary h-11 rounded-full bg-[#2B3A4A] px-6 font-medium text-white hover:bg-[#212d3a] motion-safe:active:scale-[0.98]",
    ghostBtn: "coach-ghost h-11 rounded-full px-3 text-[#2B3A4A]/75",
    link: "coach-link inline-flex items-center text-[#2B3A4A]/75 underline underline-offset-4 hover:text-[#2B3A4A] pointer-coarse:min-h-11",
    picker: "coach-picker rounded-xl border border-[#2B3A4A]/20 bg-white/85",
    surface:
      "coach-surface rounded-2xl border border-[#2B3A4A]/12 bg-white/70 shadow-[0_10px_30px_rgba(43,58,74,.08)] backdrop-blur-[1px]",
    ...
    bubble:
      "coach-bubble max-w-[85%] rounded-xl border border-[#2B3A4A]/12 bg-white/70 px-4 py-2.5 text-[15px] leading-snug whitespace-pre-wrap text-[#2B3A4A] backdrop-blur-[1px] data-[role=user]:border-transparent data-[role=user]:bg-[#2B3A4A] data-[role=user]:text-white",
  },
  copy: {
