...
| [`handoff-copy.ts`](./handoff-copy.ts) | The personalized chat hand-off copy (intro line, the three opening chips, authority notes, coach-verify line). States no academic outcome. Also the shared chip vocabulary the chat's follow-up engine reuses — the student-resource chips, the course-plan and graduation factories, `COLD_VISIT_QUESTIONS`, `MAX_LABEL`/`fitLabel` and `goalFromLabel` — because a chip is deduped by its LABEL, so a second copy of any of them in `features/chat/follow-ups.ts` would drift and offer the same question twice. |
| [`interests.ts`](./interests.ts) | `INTEREST_GUIDE` — per interest area, the example programs and faculty-expertise topics the undecided path may name. Verified against the live database; examples only, never auto-selected as the student's program. |
...
| [`onboarding-store.ts`](./onboarding-store.ts) | The **anonymous client store (#50)**: Zustand + `persist` over `localStorage` (key `student-session-store`). Holds the saved onboarding session and client-generated `studentId`; chat messages remain in the active chat only; `saveSession` / `useSavedSession` are the onboarding-facing seam. |
...
- **Client store (#50):** `onboarding-store.ts` — Zustand + `persist` to `localStorage`. `useHydrateSession()` (called once in `onboarding-flow.tsx`) reads storage on the client; reads report first-time until it has, so the server and hydration renders match, and writes before rehydration are dropped. `resetSession` is the unconsumed seam for a future "forget my answers" affordance (no owner issue yet).
...
- **Opening chips:** `starterQuestionsFor(payload)` returns exactly **three** per branch. A label is ≤ 40 characters; a prompt is a complete sentence naming the program, because the chat sends the prompt, not the label. Two shapes. A student **with a program** opens on their plan — what it requires, what they would start with, then either when those classes meet (a schedule goal or preference) or what to have ready. A student **without one** opens on their interest: the area's example programs, what one of them covers, and how two of them compare — each prompt says "example", so the model never adopts one as their program. With no interest area yet, the first chip asks for one as a numbered list the student answers with a digit. The examples come from `interests.ts`, never from the model. [`scripts/check-starter-questions.mts`](../../scripts/check-starter-questions.mts) asserts the count, the label length and the program-carrying prompts for every branch.
