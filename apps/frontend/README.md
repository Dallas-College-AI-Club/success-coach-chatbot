...
- `features/chat/suggestions.ts`: retires used suggestions and offers follow-ups from returned records. `saved-courses.ts` and `summary-sheet.tsx` provide local notes and print content.
...
npx tsc --noEmit
npm run test:demo
npx tsx scripts/check-course-code-normalization.mts
npx tsx scripts/check-sheet-question-filter.mts
npm run build
```
...
Broad expertise search returns possible CV matches, not a certified complete faculty directory. Saved CV summaries may describe earlier teaching terms. Completed-course tracking, automatic timetable construction and syllabus-policy answers are outside this release. Generated explanations remain model-dependent; exact course and schedule fields come directly from retrieved records.
