...
|---|---|---|
| Ollama (local) | `"format": <full Draft-07 schema dict>` + **always** `options.num_ctx` sized to prompt + output budget: **≥ 32768 for `program_map`** (its output budget alone is 16384), ≥ 16384 for other doc_types (8192 only for small docs on 8 GB machines) | Ollama ≥ 0.5. Without explicit `num_ctx` Ollama silently truncates at ~2–4k tokens — which presents as *plausible wrong values*, the worst failure mode. The same sizing applies to LM Studio: its **default loaded context is 4096**, which fits nothing — set context length ≥ 32768 when loading the model |
