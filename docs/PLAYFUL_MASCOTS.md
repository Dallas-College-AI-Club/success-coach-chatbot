...
- Fully concealed ground travel takes 1.15 seconds. Covered flights receive the same shortcut; visible flight speed and wing cadence are retained. Flights that remain visible above the panel are not accelerated.
...
- The engine is dynamically imported only for Playful. Decoded assets are shared between mounts; failed loads may retry. Seven WebPs total 291,924 bytes, plus a 3,454-byte vector background.
...
The five regression checks cover the accepted asset hashes and payload, 160 articulated poses per character without inverted triangles, visible collision avoidance across desktop/narrow layouts over four minutes, bounded concealed ground crossings, and continuous/reappearing flights. These checks protect geometry and routes; visual review remains necessary when changing the artwork or rig weights.
