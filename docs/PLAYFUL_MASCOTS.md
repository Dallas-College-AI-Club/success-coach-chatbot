...
- Layout measures the live header and card bounds. When room exists, flying characters explore the band above the header and ground characters roam across the meadow below the card, using separate ranges to avoid visible collisions. Route lengths, character sizes and walking/running speeds adapt on resize. In shorter windows, Bear/Thunderduck and Horse/Lion use separate side lanes. Opposite partners share a crossing clock and only pass each other behind the panel.
