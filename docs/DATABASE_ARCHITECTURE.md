...
This table is also where issue #36's named entities live: **courses** are `course` fact rows (code, title, credits, prerequisites, description — with links to their syllabi via `course_code`), and **faculty** are `cv` fact rows (name, department, email policy per the field-visibility config (see Privacy posture, §6), CV link) joined to their sections via `instructor_slug`.
...
**H. Cross-professor comparison** — unnest grading facts across a course's syllabi in one statement:
...
CROSS JOIN LATERAL jsonb_array_elements(s.facts->'grading') AS g
WHERE s.doc_type = 'syllabus' AND s.course_code = $1 AND s.year = $2 AND s.semester = $3
  AND s.facts IS NOT NULL
GROUP BY 1, 2, 3
ORDER BY 1, 4 DESC;
```
