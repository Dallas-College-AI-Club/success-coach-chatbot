...
**How to read it:** the payload lives in the anonymous client store [`onboarding-store.ts`](../../apps/frontend/features/onboarding/onboarding-store.ts) (`useSavedSession()` / `saveSession()`) — issue [#50](https://github.com/Dallas-College-AI-Club/success-coach-chatbot/issues/50)'s Zustand `persist` store, keyed `student-session-store` in `localStorage`, with a client-generated `studentId` (maps to `chat_session.student_id`); the "Welcome back" entry renders for returning students and resumes from it. The store already exposes the chat-history domain as an unconsumed floor for this work — `ChatMessage` plus `appendMessage` / `clearChat` / `updateMessageStatus` — so extend it rather than adding a parallel store, and call `useHydrateSession()` above any consumer: writes that land before rehydration are dropped. Values are closed-list (catalog ids / enum codes), so they're safe to pass straight into a prompt or a retrieval filter.
